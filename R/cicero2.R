...
#' birthdate <- '2013-10-10'
#' weight <- 80
#' cicero2(tmp, strict = FALSE)
#' \dontrun{cicero2(tmp, strict = TRUE)}
#'
...
#'     bmi <- weight/(height^2)
#'     if (bmi < 25) cicero2(x = tmpl1, name = name, surname = surname, bmi = bmi)
#'     else cicero2(x = tmpl2, name = name, surname = surname, bmi = bmi)
#' }
#'     
#' cicero2(x = select,
...
#'
#' ## cicero2 handles functions with argument ... as well
...
#' }
#' cicero2(x = f_with_dots, namefirst = TRUE, name = "mario", surname = "rossi")
#' cicero2(x = f_with_dots, namefirst = FALSE, name = "mario", surname = "rossi")
#'
#'@export
cicero2 <- function (x = NULL, strict = FALSE, ...){
